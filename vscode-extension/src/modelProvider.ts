    ...
    implements vscode.LanguageModelChatProvider<vscode.LanguageModelChatInformation>
{
    ...
    async provideLanguageModelChatResponse(
        ...
        const violationsBanner = this._formatViolationsBanner(verdict, enableAutoCorrection && maxRetries > 0);

        // In verbose/debug modes, show the original response BEFORE correction
        ...
        if (verbosity !== "minimal" && showBadges) {
            progress.report(new vscode.LanguageModelTextPart(violationsBanner));
        }
        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            ...
            if (verbosity === "debug") {
                progress.report(new vscode.LanguageModelTextPart(
                    `\n*🔧 Correction attempt ${attempt + 1}/${maxRetries}…*\n\n`,
                ));
            }
    ...
    private _formatViolationsBanner(verdict: Verdict, autoCorreecting: boolean): string {
        const lines: string[] = [
            "---\n⚠️ **aisanity intercepted violations**" +
            (autoCorreecting ? " — auto-correcting…" : "") +
            "\n\n",
        ];
